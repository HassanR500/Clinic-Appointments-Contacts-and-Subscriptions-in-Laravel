...
(function($) {
	...
	var FormFunction = function(){
				...
				var bookingForm = function () {
					jQuery('.ajax-form-two').on('submit', function(event) {
						event.preventDefault();
						var response = '<div class="alert alert-warning alert-dismissable"> Processing.. </div>';
						jQuery(this).find(".ajax-message").html(response).show('slow');
						var formData = new FormData(this);
						...
						$('.ajax-form-two .ajax-message').html(response).delay(5000).hide('slow');
						$('.ajax-form-two')[0].reset();
							},
							error: function(data) {
								var response = '<div class="err alert alert-danger">There is something wrong. Please wait or submit again.</div>';
						$('.ajax-form-two .ajax-message').html(response).delay(5000).hide('slow');
							},
						});
					});
